...
function next() {
  if (activeIndex.value !== null && activeIndex.value < sortedPhotos.value.length - 1) activeIndex.value += 1
}
function prev() {
  if (activeIndex.value !== null && activeIndex.value > 0) activeIndex.value -= 1
}
...
async function downloadOriginal(photo) {
  error.value = ''
  const response = await fetch(`${API_BASE_URL}/api/admin/image?id=${encodeURIComponent(photo.id)}&variant=original`, {
    credentials: 'include',
  })
  if (!response.ok) {
  ...
  <section class="gallery">
    ...
    <div class="grid">
      <article v-for="photo in sortedPhotos" :key="photo.id" :class="{ selected: isSelected(photo.id) }">
        <button type="button" class="image-button" @click="open(photo)">
          <img :src="photo.thumbnailUrl || photo.imageUrl" alt="" loading="lazy" />
        </button>
        <label>
          <input type="checkbox" :checked="isSelected(photo.id)" @change="toggle(photo.id)" />
          Select
        </label>
      </article>
    </div>
    ...
    <Teleport to="body">
      <div v-if="activePhoto" class="viewer">
        <div class="viewer-bar">
          ...
          <button type="button" @click="downloadOriginal(activePhoto)">Download</button>
        </div>
...
button,select { min-height:2.5rem; border:1px solid #8b6a62; background:#211916; color:#fff8ef; padding:.5rem .7rem; }
.controls { margin:1rem 0; }
.grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.65rem; }
article { position:relative; border:1px solid #4f3a34; background:#211916; }
article.selected { outline:2px solid #d9a0a7; }
...
article label { display:flex; gap:.4rem; padding:.55rem; font-size:.75rem; }
.error { color:#ffaaa1; }
.viewer { position:fixed; z-index:100; inset:0; display:grid; place-items:center; background:rgba(0,0,0,.96); padding:max(4rem,env(safe-area-inset-top)) 3.5rem max(2rem,env(safe-area-inset-bottom)); }
